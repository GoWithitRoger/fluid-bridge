...
- [Verified Fact] `src/ffi/bridge.rs` declares `extern "C"` symbols and wraps them in a `FluidAudioBridge` type used by the public APIs in `src/lib.rs`. These are Rust-to-native calls, not direct model execution in Rust tensors.
...
[Inference] The adapter should treat `fluidaudio-rs` as a platform-bound native bridge (a Rust
façade over Swift/Core ML), with dependencies on Apple Silicon/macOS/iOS Core ML execution and
FluidAudio model bootstrap/download behavior. This suits macOS and iOS apps, but it is not a
cross-platform inference runtime. It depends on external model availability, caching, and Swift
build and link behavior during installation.
